...
import com.polarbookshop.quotefunction.service.QuoteService;
...
public class QuoteFunctions {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(QuoteFunctions.class);
    ...
    Function<Mono<Genre>, Mono<Quote>> genreQuote(QuoteService quoteService) {
        return mono -> mono.flatMap(genre -> {
            LOGGER.info("Getting quote for type {}", genre);
    ...
    Consumer<Quote> logQuote() {
        return quote -> LOGGER.info("Quote: '{}' by {}", quote.content(), quote.author());
    }
}
