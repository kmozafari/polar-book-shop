package com.polarbookshop.quotefunction.service;
...
public class QuoteService {
    ...
    public Mono<Quote> getRandomQuoteByGenre(Genre genre) {
        List<Quote> quotesByGenre = quotes.stream().filter(q -> q.genre().equals(genre)).toList();
        return Mono.just(quotesByGenre.get(random.nextInt(quotesByGenre.size() - 1)));
    }
}
